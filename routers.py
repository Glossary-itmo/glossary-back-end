...
async def post_nodes(nodes: List[NodeBase]):
    ...
    if check_if_empty(fileName=file_name("node")) is True:
        return create_file(data=nodes, write_to=name, fileName=file_name("node"))
    return post_data(data=nodes, write_to=name, fileName=file_name("node"))
...
async def post_edges(edges: List[EdgeBase]):
    ...
    if check_if_empty(fileName=file_name("edge")) is True:
        return create_file(data=edges, write_to=name, fileName=file_name("edge"))
    return post_data(data=edges, write_to=name, fileName=file_name("edge"))
